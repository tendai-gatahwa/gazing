...
alert_triggered = False  # To trigger alert when looking away
...
def get_head_direction(yaw):
    global current_direction, direction_stability_counter, yaw_values, alert_triggered
    ...
    # Ensure the direction is stable over several frames before committing to a change
    ...
    if direction_stability_counter >= DIRECTION_THRESHOLD:
        current_direction = new_direction  # Change direction if it's stable over the threshold
    ...
    return current_direction, alert_triggered
...
def check_direction():
    global current_direction, alert_triggered
    return jsonify({'direction': current_direction, 'alert': alert_triggered})
